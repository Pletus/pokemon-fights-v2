...
const ClickGame: React.FC = () => {
  const [pokemons, setPokemons] = useState<PokeAPIList["results"]>([]);
  const [currentIndex, setCurrentIndex] = useState(0);
  const [score, setScore] = useState(0);
  const [timeLeft, setTimeLeft] = useState(30);

  useEffect(() => {
    axios.get<PokeAPIList>("https://pokeapi.co/api/v2/pokemon?limit=50").then((res) => {
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-10 px-4 md:px-8 flex flex-col items-center">
      ...
      {timeLeft <= 0 && <p className="text-xl font-bold mt-4">Game Over! Score: {score}</p>}
    </div>
  );
};
